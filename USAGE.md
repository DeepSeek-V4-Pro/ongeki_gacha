...
- `[pool] rotation_mode = "official"`：按官方公告日期选池，可能同时启用多个官方卡池；
...
- `/卡池` 会展示各启用池的 UP SSR 角色预览、UP/天井选择数量；`/卡池 列表` 会以转发消息列出全部 UP SSR。
...
- 帮助只保留命令列表；详细规则使用 `/规则` 查看。规则与较长列表会排版成图片卡片发送，
  图片渲染或发送失败时自动回退为原来的合并转发消息。
