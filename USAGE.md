...
- `[pool] rotation_mode = "official"`：按「几月几日」匹配历史官方池（忽略年份，用作模拟），
  可能同时启用多个官方卡池；
  `/卡池` 会列出全部启用池，默认抽取最近开启的池，也可用 `/抽卡 <池ID> <1/5/11>` 指定其他池；
  若当天没有活动池则使用常驻池（当前版本已有全部 R/SR/SSR）。
- 当前排表收录的是已抓取到的 63 个官方公告池；2023～2026 年存在大量日常池缺失，
  official 模式只按几月几日复现这些已收录池的当年同期排期，不能保证完整还原官方历史。
...
- `/卡池` 会展示各启用池的 UP SSR 角色预览、UP/天井选择数量，并附上各池官方公告图；
  `/卡池 列表` 只返回 UP 角色信息图片（池名 + 全部 UP SSR + ID），不重复发送公告图。
...
- 帮助只保留命令列表；详细规则使用 `/规则` 查看。全部指令回复统一排版为分页图片卡片，
  长内容自动分页并标注页码；图片渲染或发送失败时自动回退为合并转发或纯文本。
...
| `/天井列表` | 列出全部当前启用池的天井进度与选择卡；也可使用 `/天井 列表` |
| `/天井池 <池ID> <卡ID>` | official 模式指定某个启用中的卡池兑换天井选择卡 |
| `/规则` | 以图片卡片查看完整奖励、抽卡、天井和月卡规则，失败时回退合并转发 |
| `/卡册` | 查看收藏进度 |
| `/卡图 <ID>` | 发送已拥有卡牌的高清原图 |
| `/奖励 @用户 <点数> [备注]` | 管理员向指定用户发放点数；也可直接填写 QQ 号 |
| `/接任务 普通/挑战/终极 [音击/舞萌/中二]` | 接取音游随机任务，默认三游戏全随机；指定游戏时只从该游戏候选池抽取；普通/挑战每日次数见 `[task]` |
| `/任务列表` | 查看自己的任务、状态与剩余次数 |
| `/任务完成 <任务ID>` | 附带成绩照片提交任务，提交后等待管理员审核 |
| `/任务审核 <任务ID> <普通/S/SS/SSS/SSS+/拒绝>` | 管理员审核普通/挑战任务 |
| `/终极完成 <用户> <任务ID>` | 管理员确认终极任务完成并发放 30000 点 |
| `/任务审核列表` | 管理员查看待审核任务 |
| `/任务清理 [天数]` | 管理员立即清理已结束任务，保留天数可省略，待审核任务不会删除 |
...
- 每日签到有 `non_gacha_checkin_probability`（默认 5%）概率随机获得一张 `non_gacha_pool` 中的非抽卡卡，并发送对应卡面图；图片生成失败时回退为文字提示。
- 购买月卡消耗 1500 点，有效期 30 天，有效期内每日签到额外获得 100 点。
- 月卡剩余不超过 3 天才能续费，购买后获得两次半价五连（默认 2 次）。
- 点数达到 1000 / 3000 / 10000 时，签到或查看点数会自动领取 100 / 500 / 2000 点囤点奖励；每个档位只触发一次，默认每 60 天重置。
...
- 普通任务：每日 5 次，从三游戏全量曲库随机，游玩任意难度，奖励默认 20 点。
- 不指定游戏时默认为三游戏全随机；可在命令末尾追加 `音击`、`舞萌` 或 `中二` 指定游戏。
- 挑战任务：每日 3 次，从曲库中随机选择至少有一张 10 级或以上谱面的歌曲，
  并选取该曲中达到门槛的最低真实谱面（如舞萌 DX 的 `EXPERT DX 10`），
  任务要求为“该谱面或以上难度 S 评级”；不会锁定成该曲的最高难度谱面，
  是否挑战更高难度由玩家自己选择，不是必选项。若某曲有多张 10 级或以上谱面
  （例如 `EXPERT DX 10` 与 `MASTER DX 12`），则取其中最低的那张作为目标。
- 挑战奖励：S 30 点、SS 40 点、SSS/SSS+ 50 点。
- 终极任务：从谱面定数 ≥ 14.7 的超高难曲库随机，要求 SSS+ 评级，奖励默认
  30000 点。这里的 14.7 表示谱面内部定数不低于 14.7，不是“14 级+”。
- 终极任务只能有一个未完成/待审核；完成后该曲从个人池永久移除。
- 提交任务必须带成绩照片，由管理员在同群人工审核。
- 接取任务时若曲绘与任务卡完整，只发送图片；仅曲绘或图片生成失败时才回退文字模式。
- 曲绘下载优先使用系统 `curl`，失败时回退 Python，带重试和多 CDN 兜底。
- 签到、任务、周保底与卡池轮替均按国际时间 UTC 计算。
- 普通/挑战任务在每日 00:00 自动过期；已提交待审核任务不会被自动清理。
- 已审核、已过期、已重置的任务默认保留 30 天，随后自动清理；管理员也可使用
  `/任务清理 [天数]` 立即清理，待审核记录始终保留。
- `auto_cleanup_history` 控制是否每日自动清理，`task_history_retention_days`
  控制已结束任务保留天数；审计日志不会被清理。
- 管理员隐藏重置指令：`/任务重置 <任务ID> [备注]`，用于玩家没有对应游戏等情况。
